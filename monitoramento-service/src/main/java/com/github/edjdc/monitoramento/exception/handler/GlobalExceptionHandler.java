package com.github.edjdc.monitoramento.exception.handler;
...
import com.github.edjdc.monitoramento.exception.DuplicatedRecordException;
import com.github.edjdc.monitoramento.exception.RecordNotFoundException;
import com.github.edjdc.monitoramento.exception.handler.dto.ExceptionResponse;
import com.github.edjdc.monitoramento.exception.handler.dto.FieldError;
import com.github.edjdc.monitoramento.helper.i18n.Translator;
...
public class GlobalExceptionHandler {
	...
	public ExceptionResponse handlerObjectOptimisticLockingFailureException(ObjectOptimisticLockingFailureException ex) {
		ExceptionResponse response = new ExceptionResponse();
        response.setError(translator.get("error.record_not_updated"));
        response.setErrorDescription(translator.get("error.description.record_not_updated"));
        return response;
	}

	@ResponseStatus(HttpStatus.BAD_REQUEST)
	@ExceptionHandler(value = DataIntegrityViolationException.class)
	public ExceptionResponse handlerDataIntegrityViolationException(DataIntegrityViolationException ex) {
		ExceptionResponse response = new ExceptionResponse();
        response.setError(translator.get("error.record_not_updated"));
